...
#define CHECK(x) do { esp_err_t __ = x; if (__ != ESP_OK) return __;} while (0)
...
#define CHECK_MUX(x) do { esp_err_t __ = x; if (__ != ESP_OK) {PORT_EXIT_CRITICAL; return __;} } while (0)
...
/// @param write_data
/// @return 
inline static esp_err_t wtire_byte(ds1302_t *gpio, uint8_t write_data)
{
    uint8_t i = 0;
    for (i = 0;i < 8;i++) {
...
{
    uint8_t reg_inner;
    ESP_ERROR_CHECK(read_register(gpio, address, &reg_inner));
    return write_register(gpio, address, (reg_inner & write_mask) | write_bit);
}
...
static esp_err_t burst_write(ds1302_t *gpio, uint8_t address, uint8_t *write_data, uint8_t len)
{
    ...
    for (i = 0; i < len; i++, write_data++) {
        CHECK_MUX(wirte_byte(gpio, *write_data));
    }
...
esp_err_t ds1302_get_time(ds1302_t *gpio, struct tm *time)
{
    ...
    time->tm_mday = bcd_to_dec(cache[3]);
    time->tm_mon =  bcd_to_dec(cache[4]) - 1;
    time->tm_wday = bcd_to_dec(cache[5]) - 1;
    time->tm_year = bcd_to_dec(cache[6]) + 100; 
